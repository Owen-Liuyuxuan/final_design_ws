...
#include "rambot_msgs/DesiredTrajectory.h"
...
inline geometry_msgs::Pose2D get_relevant_pose(const geometry_msgs::Pose2D msg, const geometry_msgs::Pose2D reference_msg)
{
    ...
    result.theta = normalized_angle(msg.theta - reference_msg.theta);
...
{
    current_pose = Pose2Pose2D(msg->pose.pose);
...
void path_cb(const nav_msgs::Path::ConstPtr &msg)
{
    ...
    {
        geometry_msgs::Pose2D relevant_pose = get_relevant_pose(Pose2Pose2D(msg->poses[i].pose), current_pose);
        xvals[i] = msg->poses[i].pose.position.x;
        yvals[i] = msg->poses[i].pose.position.y;
    }
    Eigen::VectorXd coeffs = polyfit(xvals, yvals, 3);
    Eigen::VectorXd state(3);
    vector<double> result = mpc.Solve(state, coeffs, speed);
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub_handle2 = n.subscribe("/Desired_trajectory", 1, path_cb);
    ros::Subscriber sub_handle3 = n.subscribe("/odom", 1, odom_callback);
    ros::Rate loop_rate(25);
    while(!(is_path_init && is_odom_init))
    {
        ros::spinOnce();
        loop_rate.sleep();
    }
