...
public:
  ...
  double speed;
  FG_eval(Eigen::VectorXd coeffs, double speed) { this->coeffs = coeffs; this->speed = speed; }
  ...
  void operator()(ADvector &fg, const ADvector &vars)
  {
    ...
    for (int i = 0; i < N; i++)
    {
      fg[0] += i/3.0 * CppAD::pow((polyeval(coeffs, vars[x_start + i]) - vars[y_start + i]), 2);
      fg[0] += 0.4 * CppAD::pow( (CppAD::atan(vars[theta_start + i]) - derivative_eval(coeffs, vars[x_start + i])), 2);
    }
    for (int i = 0; i < N-1; i++)
    {
      fg[0] += 150 * CppAD::pow(vars[omega_start + i], 2);
    }
    for (int i = 0; i < N - 2; i++)
    {
      fg[0] += 10 * CppAD::pow(vars[omega_start + i] - vars[omega_start + i + 1], 2);
    }
  }
};
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs, float speed)
{
  bool ok = true;
  size_t i;
