...
int get_different_x_y_list(const std::vector<cv::Point2d> non_zero_point_list, Eigen::VectorXd &x_list, Eigen::VectorXd &y_list, const int number_points = 3)
{
    if (non_zero_point_list.size() <= 1.2 * number_points) {
        for (int i = 0; i < number_points; i++)
        {
            x_list[i] = non_zero_point_list[i].x;
            y_list[i] = non_zero_point_list[i].y;
        }
        return 1;
    }
    int i = 0;
    ...
    while (i < number_points && trial_time < 100 * number_points) 
    {
    ...
    if(trial_time == 10*number_points)
    {
        return 0;
    }
    return 1;
